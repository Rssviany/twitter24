        ...
        <>
            <div className='flex flex-col gap-y-4 shadow-xs md:hidden pt-4'>
                <div className='flex flex-row gap-x-3 px-2  items-center'>
                    <button
                        className={`border border-gray-500 rounded-md px-1 py-1 cursor-pointer flex items-center`}
                        ...
                        <div className='flex flex-col space-y-3'>
                            ...
                            <div className='flex flex-col gap-y-3 mb-5 md:hidden'>
                                <div className='flex flex-row justify-between '>
                                    ...
                                    <Icon icon='mdi:close'
                                        ...
                                        className='text-xl text-gray-400 hover:text-black/60'
                                    />
                                </div>
                            ...
                            {menuItems.map((item) => {
                                ...
                                return (
                                    ...
                                    <div
                                        key={item.id}
                                        className={`${active ? 'bg-[#FAF5FF] px-2 py-2 border-r-2 rounded-l-md rounded-r-xl border-[#5820f3]' : 'px-2 py-2'} flex items-center w-full`}
                                    >
                                        <div
                                            className='flex flex-row items-center gap-x-5 cursor-pointer'
                                            onClick={() => {
                                                navigate(item.moveTo);   // ✅ FIXED (no timeout)
                                                setIsOpen(false);
                                            }}
                                        >
                                            <Icon icon={item.icon} className={`text-xl ${active ? "text-[#8377ff]" : 'text-gray-600'}`} />
                                            <span className={`text-md font-semibold ${active ? "text-[#8377ff]" : 'text-black/60'}`}>
                                                {item.title}
                                            </span>
                                        </div>
                                    </div>
